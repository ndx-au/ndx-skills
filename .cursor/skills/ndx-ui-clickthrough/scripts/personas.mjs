...
export const PERSONAS = [
  {
    id: 'a11y',
    title: 'Accessibility',
    lens: 'names, roles, keyboard, focus, contrast, unlabeled controls, missing live regions',
  },
  {
    id: 'states',
    title: 'States',
    lens: 'loading, empty, error, disabled, offline vs what the plan shipped; dishonest status copy',
  },
  {
    id: 'contract',
    title: 'Contract',
    lens: 'routes, labels, IA vs ui-contract (or vs the generated draft if none existed); invented screens',
  },
  {
    id: 'visual',
    title: 'Visual',
    lens: 'layout, overflow, stacking, responsive breakpoints touched by the plan',
  },
  {
    id: 'security',
    title: 'Security',
    lens: 'UI-surface authz, XSS, secret-in-DOM, CSRF-ish clicks, open redirects',
  },
  {
    id: 'clickops',
    title: 'Click-ops',
    lens: 'live click/assert evidence from Playwright or browser-use; failed or blocked steps the plan called done',
  },
]

/** Code-review Task subagents. Parent writes clickops.json — do not spawn a Task for it. */
export const TASK_PERSONAS = PERSONAS.filter((p) => p.id !== 'clickops')

function findingSchema(ids) {
  return `{
  "persona": "${ids.join('|')}",
  "findings": [
    {
...
export const FINDING_SCHEMA = findingSchema(PERSONAS.map((p) => p.id))
/** Task prompts only. Does not advertise clickops. */
export const TASK_FINDING_SCHEMA = findingSchema(TASK_PERSONAS.map((p) => p.id))
...
export function personaPrompt(persona, phase, artifact) {
  const schema = TASK_PERSONAS.some((p) => p.id === persona.id) ? TASK_FINDING_SCHEMA : FINDING_SCHEMA
...
export function buildSuperplan({ verdict, round, maxRounds, scores, merged, artifactTitle }) {
  ...
  lines.push(
    '- After the next impl round, run `node <skill-root>/scripts/judge.mjs compose .clickthrough/inbox/review.md` on an updated review (plan + new diff). Do not invoke the gauntlet judge.',
  )
  if (round < maxRounds && verdict === 'revise' && !forcedShip) {
    ...
    lines.push('- No further clickthrough leftover loop after this round.')
  }
  lines.push('')
  return lines.join('\n')
}
